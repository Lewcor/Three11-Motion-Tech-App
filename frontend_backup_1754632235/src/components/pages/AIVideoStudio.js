...
const AIVideoStudio = () => {
  ...
  const handleDownload = (videoProject) => {
    // Create a simple HTML video structure for download
    const videoContent = `
<!DOCTYPE html>
<html>
<head>
    <title>${videoProject.title} - THREE11 MOTION TECH</title>
    <style>
        body { font-family: Arial, sans-serif; background: #f0f0f0; padding: 20px; }
        .container { max-width: 800px; margin: 0 auto; background: white; padding: 20px; border-radius: 10px; }
        .scene { margin: 20px 0; padding: 15px; border: 1px solid #ddd; border-radius: 8px; }
        .scene img { max-width: 100%; height: auto; border-radius: 5px; }
        .scene-text { margin-top: 10px; font-size: 16px; color: #333; }
        .duration { color: #666; font-size: 14px; margin-top: 5px; }
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Generated by THREE11 MOTION TECH AI Video Studio</p>
            <p>Format: ${videoFormats[videoProject.video_format]?.name} | Voice: ${videoProject.voice_style}</p>
        </div>
        ${videoProject.scenes.map((scene, index) => `
            <div class="scene">
                <h3>Scene ${index + 1}</h3>
                ${scene.image_base64 ? `<img src="data:image/png;base64,${scene.image_base64}" alt="Scene ${index + 1}" />` : ''}
                <div class="scene-text">${scene.text}</div>
                <div class="duration">Duration: ${scene.duration}s</div>
            </div>
        `).join('')}
        <div style="text-align: center; margin-top: 30px; color: #666;">
            <p>Download this file and open in a browser to view your video storyboard</p>
            <p>© THREE11 MOTION TECH - Revolutionary AI-Powered Content Creation Platform</p>
        </div>
    </div>
</body>
</html>`;
    ...
    a.href = url;
    a.download = `${videoProject.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_video_storyboard.html`;
  ...
  const handleRegenerate = async (videoProject) => {
    if (confirm('Are you sure you want to regenerate this video? This will create a new version with different images.')) {
      setIsGenerating(true);
      try {
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/video/generate`, {
          method: 'POST',
          ...
          body: JSON.stringify({
            title: `${videoProject.title} (Regenerated)`,
            script: videoProject.script,
            video_format: videoProject.video_format,
            voice_style: videoProject.voice_style,
            number_of_scenes: videoProject.scenes.length
          })
        });

        if (response.ok) {
          const newVideoProject = await response.json();
          setGeneratedVideo(newVideoProject);
          loadVideoProjects(); // Refresh projects list
          alert('Video regenerated successfully!');
        } else {
          const error = await response.json();
          alert(`Error regenerating video: ${error.detail}`);
        }
      } catch (error) {
